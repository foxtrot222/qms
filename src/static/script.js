...
// After OTP verification and redirect
function loadCustomerDetails(token) {
    if (!token) return;
    ...
    setTimeout(() => {
        ...
        if (data.success) {
            // Use the redirect URL from backend (no ?token= anymore)
...
// Call this on OTP page after verification
const tokenInput = document.getElementById("token");
if (tokenInput) {
    const token = tokenInput.value.trim();
    if (token) loadCustomerDetails(token);
}
