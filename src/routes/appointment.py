...
def get_available_slots():
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT id, strftime('%H:%M', time_slot) as time_slot FROM appointment WHERE is_booked = 0 ORDER BY time_slot")
...
def join_walkin():
    ...
    try:
        ...
        cursor.execute(f"INSERT INTO {table_name} (token_id, position, ETR) VALUES (?, ?, ?)", (token_id, next_pos, etr_formatted))
        conn.commit()
...
def book_appointment():
    ...
    try:
        cursor.execute("SELECT id FROM token WHERE value = ?", (token_value,))
        token_record = cursor.fetchone()
        if not token_record:
            return jsonify({"success": False, "error": "Invalid token."})
        token_id = token_record['id']

        cursor.execute("SELECT is_booked FROM appointment WHERE id = ? FOR UPDATE", (slot_id,))
        ...
        cursor.execute("UPDATE appointment SET is_booked = 1, token_id = %s WHERE id = ?", (token_id, slot_id))
        cursor.execute("UPDATE token SET type = 'appointment' WHERE id = ?", (token_id,))
        conn.commit()
    except Exception as e:
        conn.rollback()
        return jsonify({"success": False, "error": str(e)})
